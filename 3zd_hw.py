...
price_letters = {1:"AEIOULNSTRАВЕИНОРСТ",
                2:"DGДКЛМПУ",
                3:"BCMPБГЁЬЯ",
                4:"FHVWYЙЫ",
                5:"KЖЗХЦЧ",
                8:"JXШЭЮ",
                10:"QZФЩЪ"} #Записали стоимость букв

word = input("Введите слово: ").upper() #пользователь вводит слово
summ = 0
for i in word: #проходим по "слову", которое задал пользователь
    for k, v in price_letters.items(): #сверяемся с ключами
        if i in v: #проверяем что такой ключ есть
            summ += k  
